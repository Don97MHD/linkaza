...
export function GET(request: NextRequest) {
  ...
  const urls = slugs.map(slug => `<url><loc>${canonical(locale,slug)}</loc><xhtml:link rel="alternate" hreflang="ar" href="${canonical('ar',slug)}"/><xhtml:link rel="alternate" hreflang="en" href="${canonical('en',slug)}"/><xhtml:link rel="alternate" hreflang="x-default" href="${canonical('ar',slug)}"/></url>`).join('');
